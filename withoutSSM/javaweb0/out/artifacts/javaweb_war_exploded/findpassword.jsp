...
<c:if test="${requestScope.step==1}">
    <form style="position: absolute; left: 595px; width: 421px; top: 195px;" class="form-signin" action="findpassword" onSubmit="return account()" name="form1" method="post">
...
<c:if test="${requestScope.step==2}">
    <form style="position: absolute; left: 595px; width: 421px; top: 195px;" class="form-signin" action="findpassword" onSubmit="return yzm()" name="form2" method="post">
...
<c:if test="${requestScope.step==3}">
    <form style="position: absolute; left: 595px; width: 421px; top: 195px;" class="form-signin" action="findpassword" onSubmit="return jiance()" name="form3" method="post">
        <h1 class="h3 mb-3 font-weight-normal" >步骤三:修改新密码</h1>
        <label class="sr-only">新密码</label>
        <input type="text" name="pd" class="form-control" placeholder="新密码" required autofocus>
        <label class="sr-only">重复输入</label>
        <input type="text" name="pda" class="form-control" placeholder="重复输入" required autofocus>
        <div class="checkbox mb-3">
            <label>
            </label>
        </div>
